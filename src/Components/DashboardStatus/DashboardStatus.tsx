...
export default function DashboardStats() {
  ...
  const currentMonth = MONTHS[now.getMonth()]; // e.g., "october"
  ...
  const formatSAR = (n?: number) =>
    typeof n === "number" ? `SAR ${n.toLocaleString()}` : "SAR 0";
  ...
  const cards = useMemo(
    () => [
      { id: 1, title: "Total Client", value: formatInt(d.clients), icon: man },
      {
        ...
        value: formatInt(d.month_new_subscription),
        icon: house,
      },
      {
        id: 3,
        title: "Monthly Revenue",
        value: formatSAR(d.month_sales),
        icon: vector,
      },
      {
        id: 4,
        title: "Total Buildings",
        value: formatInt(d.month_new_added_building),
        icon: vector1,
      },
    ],
    [d]
        ...
        {isLoading
          ? Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="bg-white rounded-xl p-6 shadow">
                ...
                <div className="h-4 w-28 bg-gray-100 rounded mt-3 animate-pulse" />
